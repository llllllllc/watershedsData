...
getMinWidthBBox <- function(xy) {
  ...
  smallerSide<-as.numeric(array(NA,length(widths)))#找出長寬中較小的值
  for (i in 1:length(smallerSide)) {
    smallerSide[i]<-min(widths[i],heights[i])#找出長寬中較小的值
  }
  eMin  <- which.min(smallerSide)#找出最小寬度的長方形
  ...
  return(list(pts=pts, width=heights[eMin], height=widths[eMin], angle=deg, minWidth=smallerSide[eMin]))
}